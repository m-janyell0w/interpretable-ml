...
class GAMINet(tf.keras.Model):
    ...
    def __init__(self, meta_info, #strategy,
                 interact_num=20,
                 subnet_arch=[40] * 5,
                 interact_arch=[40] * 5,
    ...
    def fit_main_effect(self, tr_x, tr_y, val_x, val_y, sample_weight=None):
        ...
        for epoch in range(self.main_effect_epochs):
            ...
            batch_train_loss = self.evaluate(tr_x, tr_y, tr_sw,
                                                 main_effect_training=False, interaction_training=False)
            batch_val_loss = self.evaluate(val_x, val_y, sample_weight[self.val_idx],
                                                 main_effect_training=False, interaction_training=False)
            self.err_train_main_effect_training.append(batch_train_loss)
            self.err_val_main_effect_training.append(batch_val_loss)
    ...
    def fit_interaction(self, tr_x, tr_y, val_x, val_y, sample_weight=None):
        ...
        for epoch in range(self.interaction_epochs):
            ...
            batch_val_loss = self.evaluate(val_x, val_y, sample_weight[self.val_idx],
                                                 main_effect_training=False, interaction_training=False)
            self.err_train_interaction_training.append(batch_train_loss)
            self.err_val_interaction_training.append(batch_val_loss)
    ...
    def save_model(self, folder="models/gaminet/", name="model_maineffects"):
        ...
        model_dict["main_effect_weights"] = self.maineffect_blocks.weights
        model_dict["interaction_weights"] = self.interact_blocks.weights
